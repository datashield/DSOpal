...
dsadmin.package_descriptions <- function(opal, fields=NULL, df=TRUE) {
  if(is.list(opal)){
    ...
    dtos <- opalr::opal.get(opal, "datashield", "packages", query=query)
    packageList <- c()
    for (dto in dtos) {
...
dsadmin.package_description <- function(opal, pkg, fields=NULL) {
  if(is.list(opal)){
    ...
    dto <- opalr::opal.get(opal, "datashield", "package", pkg, query=query)
    packageDescription <- list()
    for (desc in dto$description) {
      packageDescription[[desc$key]] <- desc$value
    }
...
dsadmin.install_package <- function(opal, pkg, githubusername=NULL, ref=NULL) {
  if(is.list(opal)){
    ...
    opalr::opal.post(opal, "datashield", "packages", query=query)
...
dsadmin.remove_package <- function(opal, pkg) {
  if(is.list(opal)){
    ...
    resp <- opalr::opal.delete(opal, "datashield", "package", pkg, callback=function(o,r){})
  }
}
...
dsadmin.installed_package <- function(opal, pkg) {
  if(is.list(opal)){
    ...
    opalr::opal.get(opal, "datashield", "package", pkg, callback=function(o,r){
      if(r$code == 404) {
        FALSE
      } else if (r$code >= 400) {
        NULL
...
dsadmin.set_method <- function(opal, name, func=NULL, path=NULL, type="aggregate") {
  if(is.list(opal)){
    ...
    opalr::opal.post(opal, "datashield", "env", type, "methods", body=methodDto, contentType="application/json");
  }
}
...
dsadmin.rm_method <- function(opal, name, type="aggregate") {
  if(is.list(opal)){
    ...
    resp <- opalr::opal.delete(opal, "datashield", "env", type, "method", name, callback=function(o,r){})
  }
}
...
dsadmin.rm_methods <- function(opal, type=NULL) {
  if(is.list(opal)){
    ...
    if (is.null(type)) {
      ...
      resp <- opalr::opal.delete(opal, "datashield", "env", type, "methods", callback=function(o,r){})
    }
  }
}
...
dsadmin.get_method <- function(opal, name, type="aggregate") {
  if(is.list(opal)){
    ...
    m <- opalr::opal.get(opal, "datashield", "env", type, "method", name)
    class <- "function"
...
dsadmin.set_package_methods <- function(opal, pkg, type=NULL) {
  if(is.list(opal)){
    ...
    if (dsadmin.installed_package(opal,pkg)) {
      # put methods
      methods <- opalr::opal.put(opal, "datashield", "package", pkg, "methods")
      TRUE
    } else {
      FALSE
    }
...
dsadmin.rm_package_methods <- function(opal, pkg, type=NULL) {
  if(is.list(opal)) {
    ...
    # get methods
    methods <- opalr::opal.get(opal, "datashield", "package", pkg, "methods")
...
dsadmin.get_options <- function (opal) {
  if(is.list(opal)) {
    ...
    # get options
    options <- opalr::opal.get(opal, "datashield", "options")
...
dsadmin.set_option <- function (opal, name, value) {
  if(is.list(opal)) {
    ...
    ignore <- opalr::opal.post(opal, "datashield", "option", body=payload, contentType="application/json")
  }
}
...
dsadmin.rm_option <- function (opal, name) {
  if(is.list(opal)) {
    ...
    # rm option
    ignore <- opalr::opal.delete(opal, "datashield", "option", query=list(name=name))
  }
}