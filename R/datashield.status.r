  ...
  for(i in 1:length(opals)) {
    ...
    if(protocol=="https"){
      ...
      if (grepl("\\.pem$",userids[i])) {
        ...
        opals[[i]] <- opalr::opal.login(url=urls[i], opts=opal.opts)
      } else {
        opal.opts <- append(opal.opts, list(ssl.verifyhost=0, ssl.verifypeer=0))
        opals[[i]] <- opalr::opal.login(username=userids[i], password=pwds[i], url=urls[i], opts=opal.opts)
      }
    } else {
      opals[[i]] <- opalr::opal.login(username=userids[i], password=pwds[i], url=urls[i], opts=opal.opts)
    }
...
datashield.table_status<-function(opal,logins){
  ...
  for(i in seq(length(opal))){
    ...
    login_info<-try(opalr::opal.table(opal[[stdname]],datasource=datasource,table=tbl_name),silent=T)
